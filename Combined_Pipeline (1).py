...
def run_pre_processing_extract_validation_set(recording_path,current_path,params_dict):
    ...
    desired_events = ['OpenPalm', 'ActiveRest'] 
...
def crop_the_data(epochs,train_inds,validation_inds,full_epoch_tmin=0,full_epoch_tmax=5,tmin=1,tmax=2):
    ...
    train_set_data_uncroped=epochs.get_data()[:]
    train_set_labels_uncroped=epochs.events[:,-1]
...
return_df=plot_precision_recall_curves_from_trained_classifier(combined_preprocessing_dict,params_dict,precision_recall_curve_timerange=[1,2],trained_clf=trained_clf,predict_validation=True)
print(return_df)


data_set_fb = []
for filtered_data_band_epoch in combined_preprocessing_dict['filter_bank_epochs']:
     temp_data = filtered_data_band_epoch.copy().crop(tmin=2,tmax=2.5).get_data()[:]
     data_set_fb.append(temp_data)
data_set_fb_4d_array= np.transpose(np.array(data_set_fb),(1,2,3,0))
data_to_predict=(data_set_fb_4d_array)
#data_to_predict=combined_preprocessing_dict['epochs'].copy().crop(tmin=2,tmax=2.5).get_data()[:,:]
...
filtered_data_band_passed=[]
for i,(LowPass,HighPass) in enumerate(filters_bands):
    unfiltered_Raw_CSD=OriginalRaw.copy()
...
filter_bank_epochs=[]
for filtered_data_band in filtered_data_band_passed:
    epochs = mne.Epochs(filtered_data_band, events_from_annot, preload = True,baseline= None, tmin=tmin, tmax=tmax, event_id=events_trigger_dict,detrend=0)
    filter_bank_epochs.append(epochs)
# %%
#OriginalRaw_Filtered = OriginalRaw.filter(8, 32, method='iir')
events_from_annot,event_dict = mne.events_from_annotations(unfiltered_OriginalRaw)
desired_events = ['Left', 'Right'] 
events_trigger_dict = {key: event_dict[key] for key in event_dict.keys() if key in desired_events}

epochs = mne.Epochs(OriginalRaw, events_from_annot,picks=params_dict['Electorde_Group'], preload = True,baseline= None, tmin=0, tmax=4, event_id=events_trigger_dict,detrend=0)
# %%
...
baseline = (-2.5, -1.5)  # baseline interval (in s)
...
tfr = tfr_multitaper(
    combined_preprocessing_dict['epochs'].pick(["C3", "Cz", "C4"]),
    freqs=freqs,
    n_cycles=freqs,
    use_fft=True,
    return_itc=False,
